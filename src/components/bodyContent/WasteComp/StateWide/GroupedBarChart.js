  ...
  useEffect(() => {
    ...
    if (!drilldown) {
      ...
      data.children.forEach((source) => {
        ...
        source.children?.forEach((materialNode) => {
          ...
          materialMap.get(matName)[`raw_${sourceName}`] = materialNode;
        });
      });
    ...
    if (!drilldown) {
      ...
      svg
        ...
        .data(dataset)
        .join("g")
        .attr("transform", (d) => `translate(${x0(d.material)},0)`)
        .selectAll("rect")
        .data((d) =>
          sources.map((s) => ({
            key: s,
        ...
        .on("click", (event, d) => {
          ...
          setSelectedMaterial(d.material);
          setTimeout(() => setDrilldown({ name: d.material }), 200);
        })
        .transition()
        .duration(800)
        .attr("y", (d) => y(d.value))
      ...
      x0 = d3
        .scaleBand()
        .domain(dataset.map((d) => d.subMaterial))
        .range([margin.left, width - margin.right])
        .paddingInner(0.2);
      ...
      svg
        ...
        .data(dataset)
        .join("g")
        .attr("transform", (d) => `translate(${x0(d.subMaterial)},0)`)
        .selectAll("rect")
        .data((d) =>
          sources.map((s) => ({
            key: s,
    ...
    <div style={{ textAlign: "center" }}>
      <h2>
        {drilldown
          ...
          : "Material Composition by Source"}
      </h2>
      {drilldown && (
        <button
          onClick={() => {
